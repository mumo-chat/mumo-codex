...
1. Call `create_deliberation` with the user's problem. Set `application` to `"Codex"`. Set `moderator_name` to your own model identity (e.g., the model Codex is currently running as — GPT, Claude, or the configured backbone) for audit clarity — not the user's name; their identity is already on the session. Optionally set `recap_round: true` for a structured round 0 summary — see [Recap and synthesis](#recap-and-synthesis-opt-in).
