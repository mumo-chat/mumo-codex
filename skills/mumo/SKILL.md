---
name: mumo
description: Multi-model deliberation via mumo's MCP server. Best for contested architecture/product decisions, design reviews, pressure-testing a pre-launch spec, resolving tradeoffs with multiple defensible framings, or explicit user requests for a mumo panel. Requires a mumo platform API key exported as MUMO_API_KEY.
---
...
If tools return auth errors, the user needs a key. Direct them to https://mumo.chat/settings/api-keys (keys start with `mmo_live_`), then `export MUMO_API_KEY=mmo_live_...` in their shell and **restart Codex** so the env var propagates to the MCP server. After restart, the seven mumo tools become available: `create_deliberation`, `wait_for_round`, `append_round`, `get_session`, `list_sessions`, `list_models`, `get_credit`.
...
- **Subagent** — your own internal delegation infrastructure (Codex's task delegation, etc.). Subagents run in *your* harness with *your* tool access against *your* state.
...
1. Call `create_deliberation` with the user's problem. Set `application: "Codex"`.
...
Pass `moderator_name` on `create_deliberation` set to your own model identity (e.g., the model Codex is currently running as — GPT, Claude, or the configured backbone). The audit trail should reflect who's actually steering. Don't use the user's name; their identity is already on the session.
